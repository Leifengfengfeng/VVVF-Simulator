    ...
    public class vvvf_wave
    {
		...
		public static Wave_Values calculate_jre_209_mitsubishi_gto_3_level(Control_Values cv)
		{
			...
			if (!cv.brake)
			{
				...
				if (53 <= cv.wave_stat || (cv.free_run && get_Sine_Angle_Freq() > 53 * M_2PI))
				{
					...
					if (cv.free_run) amplitude = get_Amplitude(Amplitude_Mode.Level_3_1P, new Amplitude_Argument(0, 0.5, 66, 3, cv.wave_stat, false));
					else amplitude = get_Amplitude(Amplitude_Mode.Level_3_1P, new Amplitude_Argument(53, 0.83, 66, 3, cv.wave_stat, false));
				}
			...
			else
			{
				...
				if (60 <= cv.wave_stat || (cv.free_run && get_Sine_Angle_Freq() > 60 * M_2PI))
				{
					...
					if (cv.free_run) amplitude = get_Amplitude(Amplitude_Mode.Level_3_1P, new Amplitude_Argument(0, 0.5, 72, 3, cv.wave_stat, false));
					else amplitude = get_Amplitude(Amplitude_Mode.Level_3_1P, new Amplitude_Argument(60, 0.83, 72, 3, cv.wave_stat, false));
		...
		public static Wave_Values calculate_jre_e235_mitsubishi_sic_2_level(Control_Values cv)
		{
			...
			if (cv.brake)
			{
				...
				else if (86 <= cv.wave_stat || (cv.free_run && get_Sine_Angle_Freq() > 71 * M_2PI))
				{
					pulse_mode = Pulse_Mode.P_27;
					amplitude = 1.3;
				}
			...
			else
			{
				...
				else if (71 <= cv.wave_stat || (cv.free_run && get_Sine_Angle_Freq() > 71 * M_2PI))
				{
					pulse_mode = Pulse_Mode.P_27;
					amplitude = 1.3;
				}
		...
		public static Wave_Values calculate_keikyu_n1000_siemens_gto_2_level(Control_Values cv)
		{
			...
			else
			{
				...
				else if (34.5 <= cv.wave_stat || (cv.free_run && get_Sine_Angle_Freq() >= 34.5 * M_2PI))
				{
					a = 2; b = 3;
					pulse_mode = Pulse_Mode.CHMP_9;
				}
				else if (28.9 <= cv.wave_stat || (cv.free_run && get_Sine_Angle_Freq() >= 28.9 * M_2PI))
				{
					a = 2; b = 3;
					pulse_mode = Pulse_Mode.CHMP_11;
				}
				else if (24.9 <= cv.wave_stat || (cv.free_run && get_Sine_Angle_Freq() >= 24.9 * M_2PI))
				{
					a = 2; b = 3;
					pulse_mode = Pulse_Mode.CHMP_13;
				}
		...
		public static Wave_Values calculate_toubu_50050_hitachi_igbt_2_level(Control_Values cv)
		{
			...
			double amplitude = get_Amplitude(Amplitude_Mode.Linear, new Amplitude_Argument(0, 0, 61, 1, cv.wave_stat, false));
			Pulse_Mode pulse_mode;
			Carrier_Freq carrier_freq = new Carrier_Freq(700, 0);
